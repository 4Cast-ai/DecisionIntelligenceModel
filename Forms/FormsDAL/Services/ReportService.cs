    ...
    public class ReportService : BaseService
    {
        ...
        public async Task<bool> SaveCalculateScores(string report_guid, CalculateTreeData calculate_tree)
        {
            ...
            foreach (var score in allScores)
            {
                ...
                calculate_score.OrgObjGuid = score.org_obj_guid;
        ...
        public async Task<List<SavedReportDataInfo>> GetUserReports(string user_guid)
        {
            ...
            saved_report = await DbContext.SavedReports
                                  ...
                                  .OrderBy(sr => sr.Order)
                                  .Select(sr => new SavedReportDataInfo(sr, tmp, tmp, tmp, null))
                                  .ToListAsync();
        ...
        public async Task<List<ModelData>> GetOrgObjModels(string[] org_obj_guid_list)
        {
            List<ModelData> org_obj_model_list = new List<ModelData>();

            //var list = await DbContext.OrganizationObjectConnection
            //            .Where(ooc => ooc.ModelComponentGuid != null &&
            //                  org_obj_guid_list.Contains(ooc.OrgObjGuid) &&
            //                 ooc.ModelComponentGu.Status == (int)ModelComponentStatus.active &&
            //                 ooc.ModelComponentGu.Source == (int)ModelComponentSource.model_root&&
            //                 (ooc.ModelComponentGu.ModelStructureModelComponentGu==null || !ooc.ModelComponentGu.ModelStructureModelComponentGu.First().ModelComponentType.HasValue || (ModelComponentTypes)ooc.ModelComponentGu.ModelStructureModelComponentGu.First().ModelComponentType.Value != ModelComponentTypes.reference)
            //                 )
            //            .OrderBy(x => x.ModelComponentGuid)
            //            .Select(ooc => new ModelData(ooc.ModelComponentGu))
            //            .ToListAsync();

            var list = await (from ooc in DbContext.OrganizationObjectConnection
                              join mc in DbContext.ModelComponent on ooc.ModelComponentGuid equals mc.ModelComponentGuid
                              ...
                              where ooc.ModelComponentGuid != null &&
                                   org_obj_guid_list.Contains(ooc.OrgObjGuid) &&
            ...
            if (data.is_reference)
            {
                ...
                query = DbContext.Score
                          .Where(s => (s.ModelComponentGuid == data.origin_model_component_guid || s.ModelComponentGuid == rootOriginGuid) &&
                                ...
                                s.OrgObjGuid == org_obj.guid)
                          .Select(s => new ScoreData(s, s.ActivityGu, s.OrgObjGu.Name))
                          .AsEnumerable();
        ...
        private async Task<ScoreData> GetEmptyScore(string model_component_guid, UnitDataInfo org_obj, int? metric_measuring_unit, DateTime? calculate_date, string activity_guid, string origin_model_component_guid)
        {
            ...
            model_score.org_obj_name = org_obj.name;
        ...
        private async Task FillReportList(string report_guid, CalculateTreeData tree, ReportTypes report_type, UnitDataInfo org_obj, DateTime? calculate_date, bool is_root, List<string> comment_guid_list, List<string> focus_guid_list, bool hasReference)
        {
            ...
            if (string.IsNullOrEmpty(tree.data.score_data[0].org_obj_name))
                tree.data.score_data[0].org_obj_name = org_obj.name;
        ...
        private void LoopTree(CalculateTreeData tree, ReportTypes report_type, List<ModelComponentData> weaknesses_list, int i)
        {
            ...
            if (i > 2)
            {
                ...
                {
                    var s = tree.data.reference_score_list != null && tree.data.reference_score_list.Count > 0 ? tree.data.reference_score_list.FirstOrDefault(x => x.org_obj_guid == score.org_obj_guid) : null;
                    ...
                    if (IsWeakness(finalScore, tree.data.model_data.metric_measuring_unit, level.HasValue ? level.Value : 0))
                    {
                        ...
                        if (!exist)
                        {
                            ...
                            weakness.org_obj_guid = score.org_obj_guid;
        ...
        private string GetPolyGonByOrgAndModelGuid(string model_guid, string org_obj_guid)
        {
            var polygonGuid = DbContext.OrgModelPolygon
                       .Where(mc => mc.ModelComponentGuid == model_guid && mc.OrgObjGuid == org_obj_guid).FirstOrDefault()?.PolygonGuid;
        ...
        private List<ModelComponentData> GetNodeReportComment(CalculateTreeData tree, string report_guid, DateTime? date, List<string> comment_guid_list, bool hasReference)
        {
            List<ModelComponentData> comment = tree.GetNodeAndDescendants()
               ...
               .Select(s => new ModelComponentData()
               {
                   ...
                   name = hasReference ?
                                !string.IsNullOrEmpty(s.form_element_title) ? s.form_element_title + " - " + (s.model_component_comment + " - " + s.org_obj_name) : (s.model_component_comment + " - " + s.org_obj_name) :
